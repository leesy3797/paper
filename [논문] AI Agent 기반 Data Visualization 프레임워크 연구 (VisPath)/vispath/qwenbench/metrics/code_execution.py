...
def exec_code(text, timelimit=False):
    if timelimit:
        ...
        exec(text, locals())
        # exec(text, global_env)


def postprocess_code(gen_code, line, path):
    ...
    if "plt.show()" not in gen_code:
        gen_code += f"\nplt.savefig('{path}', dpi=75)\nplt.close('all')\n"

    elif 'plt.show()' in gen_code:
        gen_code.replace('plt.show()', '')
        gen_code += f"\nplt.savefig('{path}', dpi=75)\nplt.close('all')\n"

    if 'def solution()' in gen_code:
        gen_code += '\nsolution()\n'

    if ('px.' in gen_code or 'plotly' in gen_code):
        # fig 객체가 정의되어 있는지 확인
        if re.search(r'fig\s*=', gen_code):
            gen_code = gen_code.replace('fig.show()', '')  # 혹시 있으면 제거
            gen_code += f"\nimport plotly.io as pio\npio.write_image(fig, '{path}')\nimport gc\ngc.collect()\n"
        else:
            # fig가 없는 경우 사용자에게 알릴 수 있도록 주석으로 남김
            gen_code += f"\n# 'fig' 객체가 정의되지 않아 이미지를 저장할 수 없습니다.\n"
...
def eval_code_execution_rate(output_fname,
    ...
    pip_package = []
    path_list = []
    dir = os.path.splitext(output_fname)[0].rsplit('\\', 1)[0] + f'/imgs_{args.baseline}'
    print(dir)
    # dir = output_fname
    if not os.path.exists(dir):
        print(f'New Path Generated : {dir}')
        os.makedirs(dir, exist_ok=True)
    else: print(f'Alread Exists : {dir}')
    
    for _, line in enumerate(data_list):
        line_id = line['idx']
        ...
        line['code'] = gen_code
        gen_code = gen_code.replace('plt.savefig(', '# ',).replace('plt.show()', '')

        path = str(dir).replace('\\', '/') + f'/{line_id}_final.png'
        path_list.append(path)
        gen_code = postprocess_code(gen_code, line, path=path)

        if os.path.exists(path):
            print(f'{line_id}th Data has Already Code Executed. PASS ☑️')
    ...
    return code_executability, path_list
